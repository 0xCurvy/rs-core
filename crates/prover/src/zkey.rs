...
fn spot_check(pk: &ProvingKey<Bn254>) -> IoResult<()> {
    ...
    let ends_g1 = |v: &Vec<G1Affine>| v.first().map(ok_g1).unwrap_or(true) && v.last().map(ok_g1).unwrap_or(true);
...
fn deserialize_g1_vec<R: Read>(reader: &mut R, n_vars: u32) -> IoResult<Vec<G1Affine>> {
    let mut bytes = vec![0u8; n_vars as usize * G1_BYTES];
    reader.read_exact(&mut bytes)?;
    let convert = |chunk: &[u8]| deserialize_g1(&mut &chunk[..]).expect("in-memory read cannot fail");
    #[cfg(feature = "parallel")]
    return Ok(bytes.par_chunks_exact(G1_BYTES).map(convert).collect());
    #[cfg(not(feature = "parallel"))]
    Ok(bytes.chunks_exact(G1_BYTES).map(convert).collect())
}

fn deserialize_g2_vec<R: Read>(reader: &mut R, n_vars: u32) -> IoResult<Vec<G2Affine>> {
    let mut bytes = vec![0u8; n_vars as usize * G2_BYTES];
    reader.read_exact(&mut bytes)?;
    let convert = |chunk: &[u8]| deserialize_g2(&mut &chunk[..]).expect("in-memory read cannot fail");
    #[cfg(feature = "parallel")]
    return Ok(bytes.par_chunks_exact(G2_BYTES).map(convert).collect());
    #[cfg(not(feature = "parallel"))]
    Ok(bytes.chunks_exact(G2_BYTES).map(convert).collect())
}
