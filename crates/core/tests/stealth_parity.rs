//! Conformance for the stealth addressing core against committed test vectors.
//! Validates the dual-curve scheme end-to-end - including the GT-coordinate →
//! secp256k1 `b`-reduction and the BN254 / secp256k1 generators.
...
#[test]
fn send_matches_vectors() {
    let v: Vectors = serde_json::from_str(JSON).unwrap();
    assert!(!v.send.is_empty());
    for (i, s) in v.send.iter().enumerate() {
        let out = send_with_r(&s.r, &s.big_k, &s.big_v).expect("test-vector send inputs are valid");
        assert_eq!(out.big_r, s.big_r, "send R ({i})");
        assert_eq!(out.view_tag, s.view_tag, "send viewTag ({i})");
        assert_eq!(out.spending_pub_key, s.spending_pub_key, "send spendingPubKey ({i}) - b-coercion");
    }
}

#[test]
fn scan_matches_vectors() {
    let v: Vectors = serde_json::from_str(JSON).unwrap();
    assert!(!v.scan.is_empty());
    for (i, sc) in v.scan.iter().enumerate() {
        let out = scan(&sc.k, &sc.v, &sc.rs, &sc.view_tags).expect("test-vector scan inputs are valid");
        // The vectors record DENSE arrays ("" at non-matches); scan returns a sparse
        // list - rebuild the dense arrays from (index, keys) and compare.
        ...
        assert_eq!(dense_pubs, sc.spending_pub_keys, "scan spendingPubKeys ({i})");
        assert_eq!(dense_privs, sc.spending_priv_keys, "scan spendingPrivKeys ({i})");
    }
}
