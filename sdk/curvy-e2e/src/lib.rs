//! The M2 end-to-end flow, factored so both the `curvy-e2e` binary and the
//! integration test drive identical code: **shield → commit → aggregate (via blokli)
//! → scan**, against the live `poc/blokli-env` stack.
//!
//! Assumes `poc/blokli-env/run.sh up` has been run: [`run`] checks readiness and
//! returns an error with a clear message otherwise — it does NOT manage the stack.
...
// anvil dev EOAs (sign + pay gas; NOT the Curvy note-owner keys).
...
const ACC1_KEY: &str = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d"; // relayer

// Curvy account seeds (keccak-KDF'd to stealth+babyjub keys).
...
pub fn deployed_addresses() -> Result<Deployed> {
    let path = std::env::var("CURVY_ADDRESSES").unwrap_or_else(|_| {
        format!("{}/../../poc/blokli-env/curvy_deployed_addresses.json", env!("CARGO_MANIFEST_DIR"))
    });
    let raw = std::fs::read_to_string(&path).with_context(|| {
        format!("read {path} — run `poc/blokli-env/run.sh up` first (deployed-addresses file missing)")
    })?;
    let j: serde_json::Value = serde_json::from_str(&raw).context("parse deployed addresses")?;
    let get = |k: &str| -> Result<String> {
        Ok(j[k].as_str().with_context(|| format!("missing {k}"))?.to_string())
    };
    Ok(Deployed {
...
/// Quick readiness probe (bloklid `/readyz`) — used by the test to skip cleanly.
...
impl Ledger {
    ...
    fn pass(&mut self, step: &str, detail: String) {
        ...
        self.rows.push((step.to_string(), true, detail));
    }
    fn all_passed(&self) -> bool {
        self.rows.iter().all(|(_, ok, _)| *ok)
    }
    fn print(&self) {
        println!("\n================ M2 e2e ledger ================");
        for (step, ok, detail) in &self.rows {
            println!("  {:<5} {:<22} {}", if *ok { "PASS" } else { "FAIL" }, step, detail);
        }
        println!("===============================================");
    }
}

/// Run the full flow. Returns `Ok(true)` iff every step passed.
pub async fn run() -> Result<bool> {
    ...
    // ── Step 2: commit the pending note ─────────────────────────────────────────
    println!("\n[2] commitPendingNotes([noteA, 0,0,0,0])");
    let rows = client.commit(&[note_a.note_id()], ACC0_KEY, Route::Direct).await.context("commit")?;
    let state = client.anchor_state().await?;
    ledger.pass(
        "commit",
        format!("root→{}… batch {} via {}", short(&state.current_notes_root), state.current_notes_batch_index, rows[0].backend),
    );

    // ── Step 3: aggregate — spend note A, 0.3 ETH → Bob, via blokli ──────────────
    println!("\n[3] aggregate: spend noteA → {TO_BOB} to Bob (+change+fee), submit via blokli");
    let (bob_note, rows) = client
        .aggregate(&alice, &note_a, &bob.identity(), TO_BOB, ACC1_KEY, Route::Blokli)
        .await
        .context("aggregate")?;
    ledger.pass(
        "aggregate-via-blokli",
        format!("Bob note id={}… PENDING, tx {} via {}", short(&fr_to_dec(&bob_note.note_id())), short(&rows[0].tx_hash), rows[0].backend),
    );

    // ── Step 4: Bob scans + discovers his note ──────────────────────────────────
    ...
    ledger.pass("scan-discovery", format!("Bob discovered {amount} wei token {token} (decrypted, integrity-gated)"));

    // ── Stretch: Bob commits his note, then withdraws it to a fresh EOA ──────────
    const DEST: &str = "0x000000000000000000000000000000000000bEEF";
    println!("\n[5] withdrawal stretch: commit Bob's note, then withdraw to {DEST}");
    let before: u128 = client.eth_balance(DEST).await?;
    match client.commit(&[bob_note.note_id()], ACC0_KEY, Route::Direct).await {
        Ok(_) => match client.withdraw(&bob, &bob_note, DEST, ACC1_KEY, Route::Blokli).await {
            Ok((delivered, wrows)) => {
                let after: u128 = client.eth_balance(DEST).await?;
                if after.saturating_sub(before) == delivered && delivered > 0 {
                    ledger.pass(
                        "withdrawal-stretch",
                        format!("EOA +{delivered} wei (net of fee+gas), via {}", wrows[0].backend),
                    );
                } else {
                    println!("  [warn] withdrawal balance delta {} != expected {delivered}", after.saturating_sub(before));
                }
            }
            Err(e) => println!("  [warn] withdrawal not completed: {e:#}"),
        },
        Err(e) => println!("  [warn] commit-of-Bob-note failed: {e:#}"),
    }

    ledger.print();
    Ok(ledger.all_passed())
}
